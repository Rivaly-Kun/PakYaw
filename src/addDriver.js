...
document.getElementById('createDriver').onclick = async function(event) {
    ...
    try {
        ...
        for (const [id, { fileNamePath, fileName }] of Object.entries(filenameAndPath)) {
            const storageRef = ref(storage, `driverDocs/${userCredential.user.uid}_${id}_${fileNamePath}`);
        ...
        await addDoc(collection(db, 'AdminLogs'), {
            ...
            action: `User-${userId.substring(0,8)} Added Driver-${userCredential.user.uid.substring(0, 8)}`,
...
async function checkDuplicate(Firstname, Lastname, Phonenumber){
    let value = false;
    const check = query(collection(db, 'Driver'), where('first_name', '==', Firstname), 
                    where('last_name', '==', Lastname),
                    where('phone_number', '==', Phonenumber), limit(1));
    const check2 = query(collection(db, 'Driver'), where('phone_number', '==', Phonenumber), limit(1));
    const [result1, result2] = await Promise.all([getDocs(check),getDocs(check2)]);
    if(!result1.empty){
        value = true;
    }
    if(!result2.empty){
