...
import { getAuth, onAuthStateChanged, signInWithPhoneNumber, 
    signInWithCredential, GoogleAuthProvider, RecaptchaVerifier, signInWithEmailAndPassword } from "firebase/auth"
import { getFirestore, 
...
const inputElement = document.getElementById('Company');
inputElement.addEventListener('input', (event) => {
    documents.clear();
    getDocRequirements(event.target.value);
})

function getDocRequirements(param){
    let condition = param == null || param == '' ? false : true;
    console.log(condition);
    const driverReqCol = query(collection(db, 'FleetDocumentRequirements'), where('Businesses', '==', condition));
        ...
        item.innerHTML = `<div class="card">
                                    ...
                                    <div class="card-body" id="${id}">
                                        ...
                                        <div class="row" style="margin-top: 12px;">
                                            ...
                                            ${data.ExpiryDate ? `<div class="col-sm-4">
                                                <h6>Expiry Date</h6>
...
document.getElementById('createDriver').onclick = async function(event) {
    ...
    try {
        ...
        if(await checkDuplicate(company, fullname, '+63' + phone)){
        ...
        const userDocRef = doc(db, 'FleetOwner', userCredential.user.uid);
        const subCol = collection(userDocRef, 'PaymentMethods');
        const subCol2 = collection(userDocRef, 'FleetOwnerDocuments');
        const storage = getStorage();

        // Upload files
        for (const [id, { fileNamePath, fileName }] of Object.entries(filenameAndPath)) {
            const storageRef = ref(storage, `fleetOwnerDocs/${userCredential.user.uid}_${id}_${fileNamePath}`);
            await uploadBytes(storageRef, fileName);
            const downloadURL = await getDownloadURL(storageRef);
            docu[id] = {
                URL: downloadURL,
                Verified: false,
                Identification: documents.get(id).Identification ? document.getElementById(`identification-${id}`).value : null,
                IssuanceDate: documents.get(id).IssuanceDate ? Timestamp.fromDate(new Date(document.getElementById(`issuance-${id}`).value)) : null,
                ExpiryDate: documents.get(id).ExpiryDate ? Timestamp.fromDate(new Date(document.getElementById(`expiry-${id}`).value)) : null
            };
            await addDoc(collection(db, 'Notifications'), {
                user_id: userCredential.user.uid,
                message: `${company == '' ? fullname : company} submitted ${id} document for verification.`,
                resolved: false,
        ...
        await signInWithEmailAndPassword(auth, dataAdmin.data().email, dataAdmin.data().password);
        isCreatingDriver = false;
        await addDoc(collection(db, 'AdminLogs'), {
            user_id: userId,
            category: 'Add Fleet Owners',
            action: `User-${userId.substring(0,8)} Added FleetOwner-${userCredential.user.uid.substring(0, 8)}`,
...
async function checkDuplicate(Companyname, Fullname, Phonenumber){
    let value = false;
    const check = Companyname != '' ? query(collection(db, 'FleetOwner'), where('company', '==', Companyname), 
                    where('name', '==', Fullname),
                    where('phone_number', '==', Phonenumber), limit(1)) : 
                    query(collection(db, 'FleetOwner'), 
                    where('name', '==', Fullname),
                    where('phone_number', '==', Phonenumber), limit(1));
    const check2 = query(collection(db, 'FleetOwner'), where('phone_number', '==', Phonenumber), limit(1));
    const check3 = query(collection(db, 'FleetOwner'), where('company', '==', Companyname), limit(1));
    const [result1, result2, result3] = await Promise.all([getDocs(check),getDocs(check2), Companyname != '' ? getDocs(check3) : Promise.resolve({ empty: true })]);
