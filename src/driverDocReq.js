...
function renderTable(){
    ...
    documents.forEach((data, id) => {
        const row = document.createElement("tr");
            ...
            col3.innerText = data.Identification ? 'Required' : 'Not required';
            col4.innerText = data.ExpiryDate ? 'Required' : 'Not required';
            col5.innerText = data.IssuanceDate ? 'Required' : 'Not required';
            col6.innerHTML = `<button class="btn btn-warning btn-sm edit" id="editRow" data-id="${id}"><i data-feather="edit"></i></button>
...
document.getElementById('addRequiredDocument').onclick = async function(event){
    ...
    if(!form.checkValidity()){
        ...
        if(!await checkDuplicate(typeOfDoc.value)){
            await setDoc(doc(db, 'DriverDocumentRequirements', typeOfDoc.value), {
                ...
                await addDoc(collection(db, 'AdminLogs'), {
                    user_id: userId,
                    category: 'Driver Document Requirements',
...
document.getElementById('editRequiredDocument').onclick = async function(event){
    ...
    if(!form.checkValidity()){
        ...
        if(!await checkDuplicate2(typeOfDoc.value, id)){
            await deleteDoc(doc(db, 'DriverDocumentRequirements', typeOfDoc.value));
            await setDoc(doc(db, 'DriverDocumentRequirements', typeOfDoc.value), {
                Description: description,
                ExpiryDate: expiryVal,
                IssuanceDate: issuanceVal,
...
async function checkDuplicate(typeOfDoc){
    ...
    const docRef = doc(db, 'DriverDocumentRequirements', typeOfDoc);
...
document.getElementById('delete').onclick = async function(event) {
    ...
    const user = document.getElementById('User').innerText.substring(4);
    const itemId = this.getAttribute('data-id');
    const docRef = doc(db, 'DriverDocumentRequirements', itemId);
